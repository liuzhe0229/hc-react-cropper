...
const base64ToFile = (base64, fileName = 'img', mime = 'image/jpeg') => {
  const byteString = window.atob(base64)
  const content = []
  for (let i = 0; i < byteString.length; i++) {
    content[i] = byteString.charCodeAt(i)
  }
  return new window.File([new Uint8Array(content)],fileName, {type: mime})
}
...
export default { base64ToFile, imageToCanvas, canvasToBase64, size, base64ToBlob  }
